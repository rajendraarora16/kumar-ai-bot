...
git config --global user.name "kumar-gh-bot"

# Clone the git repository of kumar_server project (the gh-pages branch only as we deploy it here only)
git clone -b gh-pages --single-branch https://kumar-gh-bot:$SOURCE_UPDATER@github.com/fossasia/kumar_server.git
cd kumar_server
...
if [ "$?" = 0 ]; then
  ...
  git remote add origin-pages https://kumar-gh-bot:$SOURCE_UPDATER@github.com/fossasia/kumar_server.git
